...
When a session begins and the user sends a generic greeting ("안녕", "시작", "update", "업데이트") OR any message that does not target a specific file, run a **cheap scan** before answering.

세션 시작 시 사용자가 일반적 인사("안녕", "시작", "update", "업데이트")를 보내거나 특정 파일을 지정하지 않은 메시지를 보내면, 답변 전에 **저비용 스캔** 실행.

**Scan procedure / 스캔 절차:**

1. Identify the active hub from CWD (the deepest ancestor containing `CLAUDE.md`).
   CWD에서 활성 허브 식별 (`CLAUDE.md`를 가진 가장 가까운 조상 폴더).

2. Read ONLY the hub's `wiki/index.md` and list `raw/` filenames (research hub) or hub-root `.md` filenames (study hub). **Do not read file contents.**
   허브의 `wiki/index.md`만 읽고 `raw/` 파일명(research) 또는 허브 루트 `.md` 파일명(study) 목록화. **파일 내용은 읽지 않음.**

3. Diff the two: files not referenced in `index.md` (research) or not referenced as `derived_from:` in any `wiki/**/*.md` (study) = **new items**.
   두 목록 비교: `index.md`에 없는 파일(research) 또는 어떤 `wiki/**/*.md`에서도 `derived_from:`으로 참조 안 된 파일(study) = **새 항목**.

4. If new items found: report count + names, ask "Ingest now? / 지금 ingest할까요?" Don't auto-ingest.
   새 항목 있으면: 개수·이름 보고 + "지금 ingest할까요?" 질문. 자동 ingest 금지.

5. If none: proceed with user's request normally. Don't announce "nothing new".
   없으면: 일반 요청 처리. "새 것 없음" 선언하지 말 것.

**Cost cap / 비용 상한:** This scan = 1–2 tool calls, filename-only. If `index.md` is >500 lines, read `## Sources` section only.

**비용 상한:** 스캔 = 도구 호출 1–2회, 파일명만. `index.md`이 500줄 넘으면 `## Sources` 섹션만 읽기.

**Skip scan when / 스캔 생략 조건:**
- User's first message targets a specific file / 사용자 첫 메시지가 특정 파일 지정
- User says "no scan" / 사용자가 "스캔 생략" 요청
- CWD is vault root, not a hub / CWD가 허브가 아닌 볼트 루트
